  ...
  return (
    <>
      ...
      <div
        ...
        } pt-20 ml-4 w-100% flex flex-col gap-y-5`}
      >
        {searchResults.map((e, index) => {
          return (
            <div
              key={index * 2}
              className="flex flex-col gap-y-3 sm:flex-row gap-x-4 md:gap-x-8 w-[98%] justify-center cursor-pointer sm:mx-0"
            >
              <img
              ...
              <div className="w-[92%] sm:w-[60%] md:w-[70%] lg:w-[60%]">
                ...
                <p
                  onClick={() => pageRoute(`/watch/${e.id?.videoId}`)}
                  className="traking-wider font-normal  sm:text-[#0f0f0f] text-[13px] leading-[16px]"
                >
